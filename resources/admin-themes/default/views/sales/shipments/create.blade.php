@php
    use Webkul\Marketplace\Helpers\Shipment as ShipmentHelper;
    use Webkul\Marketplace\Helpers\RMA as RMAHelper;

    $inventorySources = app(ShipmentHelper::class)->getAllInventorySources($order);
@endphp
                            ...
                            <div class="grid">
                                ...
                                @foreach ($order->items as $item)
                                    @if (
                                        ...
                                        @php
                                            $canShipQty = app(RMAHelper::class)->getRMAStatus($item->id);
										@endphp
                                        ...
                                        @php
                                            $sellerId = $item['additional']['seller_info']['seller_id'] ?? null;
                                        
                                            $filteredInventorySources = $inventorySources->filter(fn($source) => $source->vendor_id == $sellerId);
                                        @endphp

                                        <!-- Information -->
                                        @foreach ($filteredInventorySources as $inventorySource)
