@php
    $title = $options['title'] ?? trans('kun-theme::app.home.everyones-watching');
    $subtitle = $options['subtitle'] ?? trans('kun-theme::app.home.most-viewed');
    $filters = $options['filters'] ?? [];
    $promo = $options['promo'] ?? [];
    $carouselId = 'kun-products-promo-' . $customization->id;
@endphp
...
>
    <section class="kun-section" aria-label="{{ $title }}">
        <div class="kun-promo-layout">
            <div class="kun-grid-2">
                @for ($i = 0; $i < 6; $i++)
                    <div class="shimmer rounded-[32px] h-[434px]"></div>
                @endfor
            </div>
            <div class="kun-promo-banner shimmer"></div>
        </div>
    </section>
</v-kun-products-with-promo>
...
@pushOnce('scripts')
    <script
        ...
        id="v-kun-products-with-promo-template"
    >
        <div>
            <!-- Shimmer -->
            <section class="kun-section" v-if="isLoading">
                <div class="kun-promo-layout">
                    <div class="kun-grid-2">
                        <div class="shimmer rounded-[32px] h-[434px]" v-for="n in 6" :key="n"></div>
                    </div>
                    <div class="kun-promo-banner shimmer"></div>
                </div>
            </section>
            ...
            <!-- Products + Promo -->
            <section class="kun-section kun-reveal kun-content-enter" :aria-label="title" v-else-if="products.length">
                {{-- Section Header --}}
                ...
                <div class="kun-promo-layout">
                    ...
                    {{-- Tall Promo Banner (right side) --}}
                    <div
                        v-if="promo && promo.title"
                        class="kun-promo-banner"
                        :style="{ background: promo.background || '#F5C518' }"
                    >
                        <div class="kun-promo-stripes"></div>

                        <div class="relative z-[1] py-12 px-10 flex flex-col items-center gap-6">
                            <p class="font-poppins kun-promo-title">@{{ promo.title }}</p>
                            <div class="w-4/5 h-[3px] bg-[#0F172B]"></div>
                            <p class="font-poppins kun-promo-highlight">@{{ promo.highlight }}</p>
                            <p class="font-poppins kun-promo-text">@{{ promo.text }}</p>

                            <div class="mt-8 opacity-20">
                                <svg xmlns="http://www.w3.org/2000/svg" width="120" height="120" viewBox="0 0 20 20" fill="none">
                                    <path d="M7.083 11.875c0 1.6 1.317 2.917 2.917 2.917s2.917-1.317 2.917-2.917" stroke="#0F172B" stroke-width="1" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
                                    <path d="M7.342 1.667L4.325 4.692" stroke="#0F172B" stroke-width="1" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
                                    <path d="M12.658 1.667l3.017 3.025" stroke="#0F172B" stroke-width="1" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
                                    <path d="M1.667 6.542c0-1.542.825-1.667 1.85-1.667h12.966c1.025 0 1.85.125 1.85 1.667 0 1.791-.825 1.666-1.85 1.666H3.517c-1.025 0-1.85.125-1.85-1.666z" stroke="#0F172B" stroke-width="1"/>
                                    <path d="M2.917 8.333l1.175 7.2c.266 1.617.908 2.8 3.291 2.8h5.025c2.592 0 2.975-1.133 3.275-2.7l1.4-7.3" stroke="#0F172B" stroke-width="1" stroke-linecap="round"/>
                                </svg>
                            </div>
                        </div>
                    </div>
    ...
    <script type="module">
        app.component('v-kun-products-with-promo', {
            template: '#v-kun-products-with-promo-template',

            props: ['src', 'title', 'subtitle', 'promo', 'carouselId'],
